#!/usr/bin/env python3
"""Add the PD-T4-2 system genomics trait."""
...
TARGET = REPO_ROOT / "data" / "traits" / "genomics" / "pd_t4_2_system.yaml"
...
CANONICAL_REVIEW_TIMESTAMP = "2026-09-26T15:35:01Z"
POSED_DATE = "2026-09-26"
IDENTIFIER = "traitmech:000387"
PROPOSAL = "proposals/metpo_traitmech_v264"
SLUG = "pd_t4_2"
...
WIKI_COMPOSITION_SNIPPET = (
    "The PD-T4-2 is composed of 2 proteins: PD-T4-2_A and PD-T4-2_B."
)
JEOTGALIBACA_EXAMPLE_SNIPPET = (
    "The PD-T4-2 system in *Jeotgalibaca sp. PTS2502* "
    "(GCF_001975685.1, NZ_CP019433) is composed of 2 proteins PD-T4-2_A "
    "(WP_083686895.1) PD-T4-2_B (WP_076765701.1)"
)
...
ARTICLE_REGISTRY_SNIPPET = (
    "PD-T4-2 | 10\\.1101/2022\\.05\\.12\\.491691 | Mapping the landscape "
    "of anti-phage defense mechanisms in the E\\. coli pangenome"
)
RULES_SNIPPET = (
    "PD-T4-2\tPD-T4-2\t2\t2\t"
    "PD-T4-2__PD-T4-2_A, PD-T4-2__PD-T4-2_B\t\t\t"
)
PROFILE_A = "PD-T4-2__PD-T4-2_A"
PROFILE_B = "PD-T4-2__PD-T4-2_B"
...
def vassallo_screen_evidence() -> dict[str, str]:
    return {
        ...
        "notes": (
            ...
            "selection that uncovered PD-T4-2 and related conserved "
...
def wiki_composition_evidence() -> dict[str, str]:
    return {
        ...
        "notes": "The DefenseFinder wiki names PD-T4-2 as a two-protein system.",
    }
...
def wiki_refseq_evidence() -> dict[str, str]:
    return {
        "reference": DEFENSEFINDER_WIKI,
        "snippet": JEOTGALIBACA_EXAMPLE_SNIPPET,
        "notes": (
            "The DefenseFinder wiki illustrates a predicted two-protein "
            "PD-T4-2 locus in RefSeq assembly GCF_001975685.1 on NZ_CP019433."
        ),
    }
...
def wiki_validation_evidence() -> dict[str, str]:
    return {
        ...
        "notes": (
            ...
            "Vassallo et al. to an E. coli PD-T4-2 source locus expressed "
...
def wiki_protects_evidence() -> dict[str, str]:
    return {
        ...
        "notes": (
            ...
            f"{PHAGE_LIST} in the PD-T4-2 protects-against subgraph."
        ),
    }
...
def article_registry_evidence() -> dict[str, str]:
    return {
        ...
        "notes": (
            "The DefenseFinder article registry maps the named PD-T4-2 "
...
def rules_evidence() -> dict[str, str]:
    return {
        ...
        "notes": (
            "The DefenseFinder rules table models PD-T4-2 as a two-profile "
            f"system requiring {PROFILE_NAMES}."
        ),
    }


def hmm_evidence(profile: str, row: str) -> dict[str, str]:
    return {
        "reference": DEFENSEFINDER_HMMS,
        "snippet": row,
        "notes": (
            f"The DefenseFinder HMM inventory records {profile} under the "
            "PD-T4-2 system namespace."
        ),
    }
...
RECORD: dict[str, Any] = {
    ...
    "label": "PD-T4-2 system",
    "definition": (
        "A phage defense system in which an organism possesses a PD-T4-2 "
        "locus represented by DefenseFinder as a two-profile model requiring "
        f"{PROFILE_NAMES}, and experimentally linked to {PHAGE_LIST} "
    ...
    "evidence": [
        ...
        rules_evidence(),
        hmm_evidence(PROFILE_A, HMM_ROW_A),
        hmm_evidence(PROFILE_B, HMM_ROW_B),
    ],
    "causal_graphs": [
        {
            "graph_id": "pd_t4_2_locus_restricts_phages",
            "title": f"PD-T4-2 loci restrict {PHAGE_LIST}",
            "description": (
                "Conservative system-level sketch linking possession of a "
                f"PD-T4-2 locus to protection against {PHAGE_LIST}."
            ),
            "scope_status": "NONMECHANISTIC",
            "scope_notes": (
                "The graph captures PD-T4-2 as a named DefenseFinder "
            ...
            "nodes": [
                {
                    "node_id": "pd_t4_2_locus",
                    "label": "PD-T4-2 locus",
                    "node_type": "GENETIC_ELEMENT",
                    "description": (
                        "A phage-defense locus represented by the PD-T4-2 "
                        "DefenseFinder profiles."
                    ),
                },
                {
                    "node_id": "pd_t4_2_listed_phage_protection",
                    "label": f"{PHAGE_LIST} protection",
                    "node_type": "BIOLOGICAL_PROCESS",
                    "description": f"Protection against {PHAGE_LIST} by PD-T4-2.",
                },
                {
                    ...
                    "description": (
                        "Possession of a genome-encoded PD-T4-2 "
            ...
            "edges": [
                {
                    "subject": "pd_t4_2_locus",
                    "predicate": "contributes to",
                    "predicate_id": "RO:0002326",
                    "object": "pd_t4_2_listed_phage_protection",
                    "description": (
                        "The DefenseFinder wiki links a PD-T4-2 locus "
                        f"from Vassallo et al. to protection against {PHAGE_LIST}, "
                        "and DefenseFinder models PD-T4-2 through two "
                        "mandatory profiles."
                    ),
                    "evidence": [
                        wiki_composition_evidence(),
                ...
                {
                    "subject": "pd_t4_2_listed_phage_protection",
                    "predicate": "confers",
                    "predicate_id": "METPO:2007700",
                    "object": f"{SLUG}_system_trait",
                    "description": (
                        f"Protection against {PHAGE_LIST} realizes the "
                        "PD-T4-2 system trait."
                    ),
                    "evidence": [
                        wiki_validation_evidence(),
                        wiki_protects_evidence(),
                ...
                },
                {
                    ...
                    "description": (
                        "PD-T4-2 system possession is a phage-defense-system "
                        "trait."
                    ),
                    "evidence": [
                        vassallo_screen_evidence(),
    ...
    "discussions": [
        {
            ...
            "prompt": (
                "Resolve PD-T4-2 natural host breadth, direct phage trigger, "
                "and effector mechanism before minting narrower PD-T4-2 "
                "mechanism traits."
            ),
            "kind": "KNOWLEDGE_GAP",
            "status": "OPEN",
            "rationale": (
                "Vassallo et al. support PD-T4-2 as one of the conserved "
            ...
            "attaches_to": ["causal_graphs#pd_t4_2_locus_restricts_phages"],
            "posed_by": CURATOR,
...
def write_record(*, apply: bool) -> None:
    ...
    record_curation_event(
        ...
        changes=(
            "Minted PD-T4-2 system as a DOI- and DefenseFinder-backed "
    ...
    record_curation_event(
        ...
        changes=(
            "Reviewed PD-T4-2 system canonical_examples and left them empty "
            "because the current sources support an E. coli species-level "
            "experimental validation graph, a DefenseFinder system model, "
            "and a RefSeq Jeotgalibaca sp. PTS2502 example, but not a "
            "direct native microbial isolate exemplar with experimentally "
            "verified endogenous PD-T4-2 activity. No paid research was used."
        ),
        llm_assisted=True,
