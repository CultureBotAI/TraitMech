#!/usr/bin/env python3
"""Enrich the seeded METPO filamentous colony record."""
...
TARGET = REPO_ROOT / "data" / "traits" / "other" / "filamentous_colony.yaml"
CURATOR = "codex"
TIMESTAMP = "2026-09-11T12:32:13Z"
KANDI_2015 = "DOI:10.7759/cureus.374"
OMP_FILAMENTOUS_COLONY = "http://purl.obolibrary.org/obo/OMP_0000088"

UPDATES = {
    "definition_source": OMP_FILAMENTOUS_COLONY,
    "evidence": [
        {
            ...
            "notes": (
                "Kandi lists filamentous as a bacterial colony shape value "
                "distinct from circular, irregular, and rhizoid forms."
            ),
        },
    ],
}
...
def enrich(record: dict) -> dict:
    ...
    if record.get("identifier") != "METPO:1007066":
        raise ValueError(f"expected METPO:1007066, got {record.get('identifier')!r}")
    if record.get("label") != "filamentous colony":
        raise ValueError(f"expected filamentous colony, got {record.get('label')!r}")
    ...
    record_curation_event(
        ...
        changes=(
            "Enriched the seeded METPO filamentous colony phenotype with stable "
            "OMP definition provenance and DOI-backed evidence after a "
