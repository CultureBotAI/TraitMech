#!/usr/bin/env python3
"""Enrich the seeded METPO rhizoid colony record."""
...
TARGET = REPO_ROOT / "data" / "traits" / "other" / "rhizoid_colony.yaml"
CURATOR = "codex"
TIMESTAMP = "2026-09-11T12:48:01Z"
KANDI_2015 = "DOI:10.7759/cureus.374"
OMP_RHIZOID_COLONY = "http://purl.obolibrary.org/obo/OMP_0000168"

UPDATES = {
    "definition_source": OMP_RHIZOID_COLONY,
    "evidence": [
        {
            ...
            "notes": (
                "Kandi lists rhizoid as a bacterial colony shape value distinct "
                "from circular, irregular, and filamentous forms."
            ),
        },
    ],
}
...
def enrich(record: dict) -> dict:
    ...
    if record.get("identifier") != "METPO:1007068":
        raise ValueError(f"expected METPO:1007068, got {record.get('identifier')!r}")
    if record.get("label") != "rhizoid colony":
        raise ValueError(f"expected rhizoid colony, got {record.get('label')!r}")
    ...
    record_curation_event(
        ...
        changes=(
            "Enriched the seeded METPO rhizoid colony phenotype with stable "
