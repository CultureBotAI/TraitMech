#!/usr/bin/env python3
"""Enrich the seeded METPO hemolysis record with DOI-backed evidence."""
...
TARGET = REPO_ROOT / "data" / "traits" / "other" / "hemolysis.yaml"
CURATOR = "codex"
    ...
    "evidence": [
        {
            ...
            "notes": (
                ...
                "S. aureus secretes hemolytic toxins."
            ),
        }
    ],
    "canonical_examples": [
        {
            ...
            "note": (
                "Historical S. aureus culture supernatants caused hemolysis in "
                "rabbit assays of staphylococcal toxins."
            ),
            "reference": "DOI:10.3390/toxins5061140",
        }
    ],
...
def enrich(record: dict) -> dict:
    ...
    if record.get("identifier") != "METPO:1005025":
        raise ValueError(f"expected METPO:1005025, got {record.get('identifier')!r}")
    if record.get("label") != "hemolysis":
        raise ValueError(f"expected hemolysis, got {record.get('label')!r}")
    ...
    record_curation_event(
        ...
        changes=(
            "Enriched the active METPO hemolysis class with DOI-backed "
