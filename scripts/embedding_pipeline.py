...
for(const [,,r] of displayed.slice(0,20)){{const li=document.createElement('li'),a=document.createElement('a');
a.href='../'+r.page;a.textContent=r.label+' ('+r.identifier+')';li.append(a);matches.append(li);}}}}
c.addEventListener('click',event=>{{const b=c.getBoundingClientRect(),x=(event.clientX-b.left)*devicePixelRatio,
