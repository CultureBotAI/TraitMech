#!/usr/bin/env python3
"""Add the Ogmios system genomics trait."""
...
TARGET = REPO_ROOT / "data" / "traits" / "genomics" / "ogmios_system.yaml"
...
CURATOR = "codex"
TIMESTAMP = "2026-09-19T16:33:14Z"

IDENTIFIER = "traitmech:000286"
SYSTEM = "Ogmios"
SLUG = "ogmios"
PROFILE = "VCA0308"
PROPOSAL = "proposals/metpo_traitmech_v163"
EMPTY_HMM_NAME = ""
...
RECORD: dict[str, Any] = {
    ...
    "definition": (
        ...
        f"locus represented by a {PROFILE} profile that can protect bacteria "
    ...
    "evidence": [
        ...
        },
        {
            ...
            "notes": (
                ...
                f"does not assert a resolved {SYSTEM} trigger, effector "
        ...
        },
        {
            ...
            "snippet": (
                f"{SYSTEM} | 10\\.1126/science\\.ads0768 | Sedentary "
                "chromosomal integrons as biobanks of bacterial antiphage "
                "defense systems"
            ),
            "notes": (
                "The DefenseFinder model registry maps the named "
                f"{SYSTEM} system to the Darracq et al. "
                "SCI antiphage-system paper."
            ),
        },
    ...
    "causal_graphs": [
        {
            ...
            "description": (
                f"Conservative system-level sketch linking possession of an {SYSTEM} "
                "locus to restricted bacteriophage propagation without asserting "
                "the unresolved phage trigger or molecular output."
            ),
            "scope_status": "NONMECHANISTIC",
            "scope_notes": (
                f"The graph captures {SYSTEM} as a named SCI-derived "
                f"anti-phage system with a {PROFILE} DefenseFinder profile "
            ...
            "nodes": [
                {
                    ...
                    "description": (
                        f"An {SYSTEM} anti-phage defense locus represented "
                        f"in DefenseFinder by a {PROFILE} profile."
                    ),
                },
                {
            ...
            "edges": [
                {
                    ...
                    "description": (
                        f"DefenseFinder maps {SYSTEM} to an SCI "
                        "anti-phage-system discovery paper and represents "
                        f"{SYSTEM} loci through the {PROFILE} component profile."
                    ),
                    "evidence": [
                        {
                            "reference": DARRACQ,
                            ...
                            "notes": (
                                ...
                                f"DefenseFinder {SYSTEM} registry row maps to "
                                f"the {SYSTEM} system."
                            ),
                        },
                        {
                            **hmm_inventory_evidence(),
                            "notes": (
                                ...
                                f"{PROFILE} as the {SYSTEM} component profile."
                            ),
                        },
                    ],
                },
                    ...
                    "evidence": [
                        {
                            ...
                            "snippet": (
                                ...
                                "with several providing defense through cell "
                                "lysis or growth arrest"
                            ),
                            "notes": (
                                f"Darracq et al. place the {SYSTEM} "
                                "discovery cohort in a set of novel bacterial "
                                "systems that protect against phages."
                            ),
                        },
                    ],
                },
                    ...
                    "evidence": [
                        {
                            "reference": DEFENSEFINDER_ARTICLES,
                            "snippet": (
                                f"{SYSTEM} | 10\\.1126/science\\.ads0768 | "
                                "Sedentary chromosomal integrons as biobanks "
                                "of bacterial antiphage defense systems"
                            ),
                            "notes": (
                                f"DefenseFinder associates {SYSTEM} with the "
                                "Darracq et al. bacterial antiphage-system paper."
                            ),
                        },
                    ],
                },
    ...
    "discussions": [
        {
            ...
            "status": "OPEN",
            "rationale": (
                f"Darracq et al. and DefenseFinder support {SYSTEM} as a named "
                f"anti-phage system with a {PROFILE} profile, but the trigger, "
