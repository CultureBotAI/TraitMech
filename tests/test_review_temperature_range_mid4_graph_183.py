"""Regression tests for the temperature-range-mid4 review in issue #183."""
...
from review_temperature_range_mid4_graph_183 import (  # noqa: E402
    ACTION,
    EDGE_REPLACEMENTS,
    NODE_REPLACEMENTS,
    SLUG,
...
def test_review_sources_and_grounds_warm_mesophile_edges():
    ...
    assert "membrane_lipid_composition" not in nodes
    assert "chaperone_systems" not in nodes
    assert {
        ("membrane_fluidity", "requires", "compensatory_adaptation"),
        ("rpoh_regulon", "induces", "chaperone_systems"),
    }.isdisjoint(by_key)
...
def test_refuses_partial_replay():
    doc = _before()
    graph = doc["causal_graphs"][0]
    replacement = EDGE_REPLACEMENTS[7]
