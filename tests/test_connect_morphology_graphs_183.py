"""Regression tests for the first morphology tranche of issue #183."""
...
from connect_morphology_graphs_183 import (  # noqa: E402
    ACTION,
    ADDITIONS,
    EXPECTED_COMPONENTS,
    REMOVALS,
...
def _before(slug: str) -> dict:
    ...
    if slug == "morphology/flagellated":
        graph["nodes"].extend([
            {
                "node_id": "ion_motive_force",
                "label": "ion motive force",
                "node_type": "BIOLOGICAL_PROCESS",
            },
            {
                "node_id": "proton_channel",
                "label": "proton channel",
                "node_type": "MOLECULAR_FUNCTION",
            },
            {
                "node_id": "stator_complex",
                "label": "stator complex",
                "node_type": "GENE_OR_PROTEIN",
            },
        ])
        graph["edges"].extend([
            {
                "subject": "ion_motive_force",
                "predicate": "powers",
                "object": "stator_complex",
                "evidence": [{"reference": "DOI:10.3389/fmicb.2021.659464"}],
            },
            {
                "subject": "stator_complex",
                "predicate": "has function",
                "object": "proton_channel",
                "evidence": [{"reference": "DOI:10.3390/biom14121488"}],
            },
        ])
    return doc
...
def test_flagellar_motor_island_is_removed_as_out_of_scope():
    slug = "morphology/flagellated"
    doc = _before(slug)
...
def test_refuses_connector_metadata_drift_after_migration():
    slug = "morphology/brown_pigmented"
    doc = _current(slug)
    graph = doc["causal_graphs"][0]
