"""Regression tests for the temperature-range-very-low review in issue #183."""
...
from review_temperature_range_very_low_graph_183 import (  # noqa: E402
    ACTION,
    EDGE_REPLACEMENTS,
    NODE_REPLACEMENTS,
    SLUG,
...
def test_review_adds_snippets_and_grounds_psychrophile_range_edges():
    ...
    assert "rna_secondary_structure" not in nodes
    assert "cold_stabilized_rna_secondary_structure" in nodes
    assert {
        ("cold_shock_proteins", "acts on", "rna_secondary_structure"),
        ("molecular_chaperones", "preserves", "proteostasis_cold"),
        ("compatible_solutes", "stabilizes", "protein_membrane_stabilization"),
    }.isdisjoint(by_key)
