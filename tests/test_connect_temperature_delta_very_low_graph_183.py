"""Regression tests for the temperature_delta_very_low component repair in issue #183."""
...
import connect_temperature_delta_very_low_graph_183 as migration  # noqa: E402
from connect_temperature_delta_very_low_graph_183 import (  # noqa: E402
    ACTION,
    ADDED_EDGES,
    ADDED_NODES,
    EXPECTED_COMPONENTS,
...
def _before() -> dict:
    ...
    added_node_ids = {node["node_id"] for node in ADDED_NODES}
    added_edge_keys = {_edge_key(edge) for edge in ADDED_EDGES}

    graph.update(GRAPH_METADATA_BEFORE)
    graph["nodes"] = [
        copy.deepcopy(node)
        for node in graph["nodes"]
        if node["node_id"] not in added_node_ids
    ]
    graph["edges"] = [
        copy.deepcopy(edge)
        for edge in graph["edges"]
        if _edge_key(edge) not in added_edge_keys
    ]
    return doc
...
def test_repair_reaches_one_component_with_exact_snippet_backed_edges():
    ...
    for expected in ADDED_NODES:
        assert nodes[expected["node_id"]] == expected
...
def test_refuses_added_node_drift_after_migration():
    doc = _transform_from_before()
    graph = doc["causal_graphs"][0]
    node_id = ADDED_NODES[0]["node_id"]
...
def test_refuses_missing_added_endpoint(monkeypatch: pytest.MonkeyPatch):
    doc = _before()
    monkeypatch.setattr(migration, "ADDED_NODES", ADDED_NODES[1:])
...
def test_refuses_partial_replay():
    doc = _before()
    doc["causal_graphs"][0]["nodes"].append(copy.deepcopy(ADDED_NODES[0]))
