"""Regression tests for the NaCl-delta-mid2 review in issue #183."""
...
from review_nacl_delta_mid2_graph_183 import (  # noqa: E402
    ACTION,
    EDGE_REPLACEMENTS,
    NODE_REPLACEMENTS,
    SLUG,
...
def test_review_adds_snippets_and_repairs_nacl_delta_mid2_graph():
...
def test_refuses_partial_replay():
    doc = _before()
    graph = doc["causal_graphs"][0]
    replacement = NODE_REPLACEMENTS[0]
    graph["nodes"] = [
        copy.deepcopy(replacement["after"]) if node == replacement["before"] else node
        for node in graph["nodes"]
    ]
